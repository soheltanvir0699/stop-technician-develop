...
class MyBidViewController: StopAppBaseView, IndicatorInfoProvider {

    // MARK: - Indicator info provider
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: GusSetLanguage.getLanguage(key: "menu.work.mybid"))
    }
    ...
    override func viewDidLoad() {
        ...
        if BaseDelegate.isReceivedNotification {
            self.performSegue(withIdentifier: "detailbidding", sender: self)
        } else {
            self.bindViewModel()
        }
...
extension MyBidViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "detailbidding" {
            let destination = segue.destination as! DetailBiddingViewController
            if BaseDelegate.isReceivedNotification {
                destination.id = BaseDelegate.idOrder
            } else {
