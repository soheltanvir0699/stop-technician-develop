...
public var APIKEY:String = "AIzaSyDd68pEV8ERaFkPV1W_QYbfrIVBpUneNB4"
...
public class BuildDirection: NSObject, URLSessionDataDelegate {
     ...
     *
     */
    public typealias completionHandler = (_ route: GoogleDirections) -> Void
    ...
    open func directionCompletion(handler: @escaping (_ route: GoogleDirections) -> Void,
...
extension BuildDirection {
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if self.response?.statusCode == 200 {
            ...
            let direction = try! JSONDecoder().decode(GoogleDirections.self, from: self.data)
            
            /*
             * Success
             */
            if direction.errorMessage == nil && direction.status != "ZERO_RESULTS" {
                self.completion!(direction)
            } else if direction.errorMessage == nil || direction.status == "ZERO_RESULTS" || direction.status == "MAX_ROUTE_LENGTH_EXCEEDED" {
                let err = NSError(domain: "Status: \(direction.status!)", code: 10058)
               self.failuer!(err)
            } else{// Failuer handling
                let err = NSError(domain: "\(direction.errorMessage!)\nStatus: \(direction.status!)", code: 10058)
                self.failuer!(err)
            }
        }else{//Error Handling
...
public extension GMSMapView {
    ...
    func addDirection (path: String, color: UIColor = .blue) {
        ...
        line.strokeColor = UIColor.hexStringToUIColor(hex: "E42F38")
